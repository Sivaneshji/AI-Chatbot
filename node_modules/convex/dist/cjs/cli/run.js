...
const run = new import_extra_typings.Command("run").description("Run a function (query, mutation, or action) on your deployment").allowExcessArguments(false).argument(
...
).addOption(new import_extra_typings.Option("--no-push").hideHelp()).addDeploymentSelectionOptions((0, import_command.actionDescription)("Run the function on")).addOption(
  new import_extra_typings.Option(
    "--typecheck <mode>",
  ...
  if (options.watch) {
    return await (0, import_run.subscribeAndLog)(ctx, {
      deploymentUrl,
      adminKey,
      functionName,
  ...
  return await (0, import_run.runFunctionAndLog)(ctx, {
    deploymentUrl,
    adminKey,
    functionName,
    argsString: argsString ?? "{}",
